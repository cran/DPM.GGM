...
int Graph::GenerateAllCliques()
{
	...
	{		
		...
		if(CheckCliques() < 0)
		{
    		        //----- AFL Addition to fix memory leak
			for(i=0; i<n; i++)
			  {
			    delete[] AllCliques[i];
			    AllCliques[i] = NULL;
...
extern "C"{
  void is_decomposable(int *es,int *p,int *result,int *cliques,int *cldims,
		       int *ncliques,int *seps, int *sepdims,int *nseps)
  {
    int i,j;
    LPGraph graph = new Graph;
    graph->InitGraph(*p);
    for(i=0;i<*p;i++){for(j=0;j<*p;j++){
      graph->Edge[i][j]=es[i * *p +j];
    }}
    *result=graph->IsDecomposable();
    if(*result==1){
      for(i=0;i<*p;i++){
	cldims[i]=graph->CliquesDimens[i];
	sepdims[i]=graph->SeparatorsDimens[i];
	for(j=0;j<*p;j++){
	  cliques[i * *p+j]=graph->Cliques[i][j];
	  seps[i * *p +j]=graph->Separators[i][j];
	}
      }
      *ncliques=graph->nCliques;
      *nseps=graph->nSeparators;
    }
    delete graph;
    return;
  }
...
extern "C"{
  void make_fill_in(int *A, int *p)
  {
	...
	for(j = 0; j < *p; j++)
	  {
	    graph->Edge[i][j] = A[i * *p +j];
	  }
      }
    LPGraph fillingraph = MakeFillInGraph(graph);
    
    for(i = 0; i < *p; i++)
      {
	for(j = 0; j < *p; j++)
	  {
	    A[i * *p + j] = fillingraph->Edge[i][j];
	  }
      }
    delete graph;
    delete fillingraph;
...
extern "C"{
  void get_mpsub_R(int *A,int *p,int *cliques,int *cldims,int *decomposable,
		       int *ncliques,int *seps, int *sepdims,int *nseps){
    int i,j;
    LPGraph graph = new Graph;
    graph->InitGraph(*p);
